...
class ErrorCode(str, Enum):
    ...
    """
    # 意图解析类错误（E01xx）：用于承接第1组 1001~1004 错误，
    # 避免与第五组既有的 E1001 安全拒绝发生编号和语义冲突。
    E0101 = "E0101"   # 无法识别用户意图
    E0102 = "E0102"   # 意图参数不完整，需要用户补充
    E0103 = "E0103"   # HostAgent 或模型服务不可用
    E0104 = "E0104"   # HostAgent 输出格式无效

    # 安全类错误（E1xxx）
    E1001 = "E1001"   # 安全检查拒绝（危险操作/受保护路径）
    E1002 = "E1002"   # 路径穿越攻击检测
    E1101 = "E1101"   # 高风险操作需要用户确认
    E1102 = "E1102"   # 用户拒绝确认或确认已失效

    # 规划类错误（E2xxx）
    E2001 = "E2001"   # 规划模块未注册/不可用
    E2002 = "E2002"   # 规划结果格式无效
    E2003 = "E2003"   # 控件检测能力不可用
    E2004 = "E2004"   # 控件检测结果格式无效

    # 执行类错误（E3xxx）
    E3001 = "E3001"   # 执行模块未注册/不可用
    E3002 = "E3002"   # 执行结果格式无效
    E3003 = "E3003"   # 控件重新检测后执行仍无法恢复
    E3004 = "E3004"   # 执行检查点缺失或无效

    # 工具调用类错误（E4xxx）
    E4001 = "E4001"   # 工具注册表未注册/不可用
    E4002 = "E4002"   # 工具调用失败
    E4003 = "E4003"   # 工具请求格式无效
    E4004 = "E4004"   # 工具调用ID重复或已消费

    # 系统类错误（E5xxx）
    E5001 = "E5001"   # 输入参数校验失败
    E5002 = "E5002"   # 模块调用超时（超过 3s）
    E5003 = "E5003"   # 未知内部错误
    E5004 = "E5004"   # 用户取消正在运行的任务


# 错误码对应的中文描述字典
ERROR_MESSAGES: Dict[ErrorCode, str] = {
    # 意图解析类
    ErrorCode.E0101: "意图解析失败：无法识别用户意图",
    ErrorCode.E0102: "意图解析暂停：缺少完成任务所需参数",
    ErrorCode.E0103: "意图解析失败：HostAgent 或模型服务不可用",
    ErrorCode.E0104: "意图解析失败：HostAgent 返回格式无效",
    # 安全类
    ErrorCode.E1001: "安全检查拒绝：检测到危险操作或受保护路径访问",
    ErrorCode.E1002: "安全检查拒绝：检测到路径穿越攻击（../ 序列）",
    ErrorCode.E1101: "高风险操作暂停：需要用户明确确认",
    ErrorCode.E1102: "高风险操作未获授权或确认已失效",
    # 规划类
    ErrorCode.E2001: "规划模块（Group2）未注册或服务不可用",
    ErrorCode.E2002: "规划模块返回结果格式无效，缺少必要字段",
    ErrorCode.E2003: "控件检测模块（Group2）不可用",
    ErrorCode.E2004: "控件检测模块返回结果格式无效",
    # 执行类
    ErrorCode.E3001: "执行模块（Group3）未注册或服务不可用",
    ErrorCode.E3002: "执行模块返回结果格式无效，缺少必要字段",
    ErrorCode.E3003: "执行恢复失败：重新检测后控件仍不可用",
    ErrorCode.E3004: "执行恢复失败：检查点缺失或无效",
    # 工具类
    ErrorCode.E4001: "工具注册表（Group4）未注册或服务不可用",
    ErrorCode.E4002: "工具调用失败，工具执行过程中发生错误",
    ErrorCode.E4003: "工具请求格式无效",
    ErrorCode.E4004: "工具调用已执行或正在执行，拒绝重复提交",
    # 系统类
    ErrorCode.E5001: "输入参数校验失败：缺少必要字段或类型错误",
    ErrorCode.E5002: "模块调用超时：响应时间超过 3 秒阈值",
    ErrorCode.E5003: "系统内部未知错误，请查看审计日志排查",
    ErrorCode.E5004: "任务已由用户取消",
}
